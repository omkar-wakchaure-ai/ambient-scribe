...
def run_pipeline(
    ...
    # process_audio() reports the audio-stage progress itself
    # (Transcribing audio 15% -> Running speaker diarization 35% ->
    #  Aligning speakers 45%), so the UI never stalls on one step.
    transcript = process_audio(
        audio_file, language=language, on_progress=report
    )
