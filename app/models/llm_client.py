...
def _call_groq(system_prompt: str, user_prompt: str, model_alias: str,
    ...
    # Transient failures (rate limits, API 5xx, timeouts, connection drops)
    # are retried with exponential backoff so the post-processing pipeline
    # (SOAP + action summary) survives temporary Groq outages instead of
    # killing the whole consultation job.
    max_retries = int(os.environ.get("GROQ_MAX_RETRIES", "5"))
    base_delay = float(os.environ.get("GROQ_RETRY_BASE_DELAY", "1.5"))
    ...
    for attempt in range(max_retries):
        try:
            ...
            if isinstance(exc, RateLimitError):
                ...
                hint = " (Groq server error)"
            else:
                hint = " (connection/timeout)"
            print(
                f"Groq {model_alias!r} request {attempt + 1}/{max_retries} "
                f"failed{hint}; retrying in {delay:.1f}s"
            )
            time.sleep(delay)
    ...
    raise RuntimeError(
        f"Groq request failed after {max_retries} retries "
        f"(model={model}, alias={model_alias!r}): {last_exc}"
    ) from last_exc
