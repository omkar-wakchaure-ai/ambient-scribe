...
class JobManager:
    ...
    def create_job(self, patient_id: Optional[str] = None,
                   appointment_id: Optional[str] = None) -> tuple[str, dict]:
    ...
    def list_for_patient(self, patient_id: str) -> list[dict]:
        """Return all jobs bound to one patient (for doctor/patient report
        sync and leakage prevention)."""
        matches = []
        with self._lock:
            for job in self._jobs.values():
                if job.get("patient_id") == patient_id:
                    matches.append(dict(job))
        return sorted(matches, key=lambda j: j.get("updated_at", 0), reverse=True)
