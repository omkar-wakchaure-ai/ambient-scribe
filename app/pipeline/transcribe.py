...
Credentials: if the configured model is a gated/private Hugging Face repo,
the ``HF_TOKEN`` / ``HUGGINGFACE_TOKEN`` value from the project ``.env`` is
passed along as ``use_auth_token`` so model downloads do not fail with a
