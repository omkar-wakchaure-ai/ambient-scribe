...
async def create_consultation(
    ...
    appointment_id: str = Form(default=None),
):
    """Upload an audio file and start the full A -> B pipeline in the
    background. The job is strictly bound to ``patient_id`` (and, when
    given, ``appointment_id``) so one patient's audio/transcript/report can
    never leak into another patient's view. Poll
    ``GET /consultations/{job_id}`` for progress."""
    if not patient_id or not patient_id.strip():
        raise HTTPException(
            status_code=400,
            detail=(
                "patient_id is required. Every voice submission must be "
                "bound to a patient."
            ),
        )
...
def api_status():
    ...
    from app.config import is_huggingface_token_configured
    ...
    return {
        ...
        "hf_token_configured": is_huggingface_token_configured(),
    }
...
def _matches_patient(job: dict, patient_id: str) -> bool:
    """Enforce patient isolation on job accessors.

    If a caller claims a ``patient_id``, it must match the patient the job
    was created for. A mismatch is treated as "not found" so a leak attempt
    gets the same 404 as an unknown job id.
    """
    if not patient_id:
        return True
    return job.get("patient_id") == patient_id
...
def consultation_status(
    ...
    job = _get_job(job_id)
    if not _matches_patient(job, patient_id):
        raise HTTPException(status_code=404, detail="Job not found")
    return JobStatusResponse(**job)
...
def job_transcript(
    ...
    if not _matches_patient(job, patient_id):
        raise HTTPException(status_code=404, detail="Job not found")
    result = _require_completed(job)
    return {"job_id": job_id, "patient_id": job.get("patient_id"), "segments": result.get("transcript", [])}
...
# ----------------------------------------------------------------------
# Patient-scoped reports (doctor dashboard sync + patient records view)
# ----------------------------------------------------------------------

@router.get("/patients/{patient_id}/consultations", response_model=PatientReportsResponse)
def patient_consultations(patient_id: str):
    """Every consultation job bound to ``patient_id``.

    Backend-enforced isolation: results are keyed strictly by ``patient_id``
    so cross-patient leakage is impossible through this endpoint. It powers
    the doctor's patient-history reports section and the patient's own
    records view.
    """
    reports = []
    for job in job_manager.list_for_patient(patient_id):
