...
import { ensurePatientReports } from '../../services/patientStore';
...
export default function BookAppointment() {
  ...
  // Every appointment and every subsequently-generated report is scoped to
  // the logged-in patient's identity.
  const patientId = user?.email || 'guest-patient';
  ...
  const handleConfirmAppointment = () => {
    ...
    localStorage.setItem(
      'demo_appointments',
      JSON.stringify([...(Array.isArray(existingAppts) ? existingAppts : []), newAppt])
    );

    // New patients start with an EMPTY reports/history collection by default.
    ensurePatientReports(patientId);
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0 pb-6">
          ...
          <div className="lg:col-span-7 flex flex-col gap-6 h-full">
            ...
            <div className="flex-1 min-h-0">
              <PreVisitAudioRecorder patientId={patientId} />
            </div>
          </div>
