...
import { CalendarPlus, FileText, LogOut, X, ChevronDown, ChevronUp } from 'lucide-react';
import { useAuth } from '../../context/AuthContext'; // Import the auth context
import DocumentViewer from '../../components/doctor/DocumentViewer';
import { getReports } from '../../services/patientStore';
...
export default function PatientDashboard() {
  ...
  const loadDocuments = () => {
    ...
    // Reports are strictly scoped to this patient's identity.
    setReports(getReports(patientEmail));
  };
          ...
          <div className="relative z-10 w-full max-w-lg bg-white/95 backdrop-blur-xl rounded-[2rem] border border-white shadow-[0_20px_60px_rgba(0,0,0,0.15)] overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-[#E5E7EB]/50">
              <div>
                ...
                <p className="text-xs text-[#64748B] mt-1">My consultation reports and uploaded documents.</p>
              </div>
              <button
            ...
            <div className="max-h-[55vh] overflow-y-auto p-4 space-y-4">
              ...
              {reports.length > 0 && (
                <div>
                  <h4 className="text-sm font-bold text-[#172033] px-4 py-2 tracking-tight">Consultation Reports</h4>
                  <div className="space-y-2">
                    {reports.map((report) => {
                      const rid = report.id || report.jobId || report.date;
                      const open = openReport === rid;
                      return (
                        <div key={rid} className="bg-[#F5F9FF] border border-[#E0F2FE] rounded-2xl overflow-hidden">
                          <button
                            onClick={() => setOpenReport(open ? null : rid)}
                            className="w-full flex items-center justify-between px-4 py-3 text-left"
                          >
                            <div className="min-w-0">
                              <p className="text-sm font-bold text-[#172033] truncate">SOAP Consultation Report</p>
                              <p className="text-[11px] font-medium text-[#64748B] mt-0.5">
                                {report.date} · {report.jobId ? `Job ${report.jobId}` : 'saved locally'}
                              </p>
                            </div>
                            {open ? (
                              <ChevronUp className="w-4 h-4 text-[#4F46E5] flex-shrink-0" />
                            ) : (
                              <ChevronDown className="w-4 h-4 text-[#4F46E5] flex-shrink-0" />
                            )}
                          </button>
                          {open && (
                            <div className="px-4 pb-4">
                              <pre className="text-xs text-[#172033] leading-relaxed bg-white border border-[#E0F2FE] rounded-xl p-3 whitespace-pre-wrap font-sans">
