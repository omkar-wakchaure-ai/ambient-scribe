...
import { addReport } from '../../services/patientStore';
...
const PROCESSING_STEPS = [
  ...
  'Aligning speakers and merging turns',
...
export default function LiveConsultation() {
  ...
  // Every voice submission/report is bound to this patient id so one
  // patient's data never leaks into another patient's session.
  const patientId = (
    currentPatient?.patientId ||
    currentPatient?.patientEmail ||
    (currentPatient?.id != null ? `patient-${currentPatient.id}` : '') ||
    'guest-patient'
  );
  const appointmentId = currentPatient?.id != null ? String(currentPatient.id) : patientId;
  const {
    status,
    setStatus,
    jobId,
  ...
  const normalizedStep = (step) => {
    ...
    if (s.includes('align') || s.includes('merg')) return 'Aligning speakers and merging turns';
  ...
  const runConsultation = async (audioFile, sourceName) => {
    ...
    setIsUploading(true);
    try {
      const uploaded = await createConsultation(audioFile, language, patientId, appointmentId);
      ...
      const [transcript, soapNote, actions] = await Promise.all([
        fetchTranscript(id, patientId),
        fetchSoapNote(id, patientId),
        fetchActionSummary(id, patientId),
      ]);
  ...
  const handleEndVisit = () => {
    // 1) Save the finished consultation to THIS patient's medical reports so
    //    it appears on the patient profile / patient records view.
    if (patientId && clinicalData) {
      addReport(patientId, {
        id: `${clinicalData.jobId || Date.now()}`,
        jobId: clinicalData.jobId || null,
        patientId,
        appointmentId: appointmentId || null,
        date: new Date().toLocaleString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }),
        soap_note: clinicalData.soap_note || '',
        extraction: clinicalData.extraction || {},
        actions: clinicalData.actions || {},
        segments: clinicalData.segments || [],
      });
    }

    // 2) Mark the appointment COMPLETED (kept in storage, but the dashboard
    //    grid filters COMPLETED out of "Upcoming Appointments").
    let appts = [];
    try {
    ...
    if (currentId === undefined && appts.length > 0) {
    ...
    const updated = appts.map((a) => (
      String(a.id) === String(currentId)
        ? { ...a, status: 'COMPLETED', patientId: a.patientId || patientId }
        : a
    ));
    localStorage.setItem('demo_appointments', JSON.stringify(updated));

    // 3) Increment the Completed Notes stat on the dashboard.
